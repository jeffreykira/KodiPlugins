...
def download(info, path, template='%(title)s-%(id)s.%(ext)s'):
    ...
    try:
        AddonSignals.sendSignal('download.started', signalPayload, sourceID='script.oceanktv.youtube.dl')
        ...
        AddonSignals.sendSignal('download.finished', signalPayload, sourceID='script.oceanktv.youtube.dl')
...
def manageDownloads():
    ...
    """
    xbmc.executebuiltin('RunScript(script.oceanktv.youtube.dl)')
